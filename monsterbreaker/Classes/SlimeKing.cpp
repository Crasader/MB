...
SlimeKing::SlimeKing()
{
	_name = "SlimeKing";
	_type = "Monster";
	_bossName = "Slime King";
	stayTime = 1.0f;
	attackTime = 3.0f;
	moveTime = 2.0f;
}
SlimeKing* SlimeKing::create()
{
	auto pSprite = new SlimeKing();
...
void SlimeKing::MoodAttack(float dt)
{
	if (timer == kTimerUp)
	{
		timer = AttackTime();
...
void SlimeKing::MoodMove(float dt)
{
	if (timer == kTimerUp)
	{
		timer = MoveTime();
...
void SlimeKing::MoodStay(float dt)
{
	if (timer == kTimerUp)
	{
		timer = StayTime();
...
void SlimeKing::MakeTheOtherSelf()
{
	auto s1 = SlimeKingMiddle::create();
	auto s2 = SlimeKingMiddle::create();
